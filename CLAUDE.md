...
- **Yoga-illustrasjoner, to runder (2026-07-24 → 2026-07-25).** Runde 1: strekfigurer (tynne linjer + leddprikker) ble avvist av brukeren («Vi kan ikke ha strekfigurer») og erstattet med HELT abstrakte animerte glyffer (buer, ringer — samme motiv som app-logoen). Runde 2: de abstrakte glyffene viste seg IGJEN å være for vanskelige å tolke («Kan du lage noen figurer som ligner på mennesker»). **Endelig løsning:** fylte hoder + tykke, avrundede strøk for torso/lemmer (ikke tynne linjer, ikke leddprikker) — likner hvordan treningsapper (Nike Training, Down Dog) tegner forenklede positur-ikoner. **How to apply:** ikke gå tilbake til tynne strekfigurer ELLER ren abstraksjon for denne appen — «fylt/avrundet menneskefigur» er den vurderte riktige balansen. Bevisst IKKE hentet ikoner fra nettet (uklar lisensiering for kommersiell App Store-bruk) — alle 9 positurer er håndtegnet i `POSE_PATHS`, 100% rettighetsfrie. Samtidig kortet ned positur-varighetene fra 45-60 sek til 15-30 sek (brukeren: «umulig å måtte holde disse positurene så lenge») i `YOGA_ROUTINES_DEF`.
