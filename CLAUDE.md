...
- **Yoga-strekfigurer fjernet helt (2026-07-25) — brukeren avviste dem eksplisitt («Vi kan ikke ha strekfigurer»).** `POSE_PATHS` erstattet med abstrakte animerte glyffer i samme visuelle språk som appens egne ikoner (buer, ringer) — f.eks. konsentriske ringer for barnets positur (bevisst gjenbruk av selve app-logo-motivet), en stigende kurve for kobra. Alle pulserer rolig likt pusteøvelse-orben (`.yogaGlyph`/`@keyframes yogaGlyphPulse`). Teksten (`POSE_CUES`) bærer nå hele instruksjonsjobben, glyffen er bare stemning — **How to apply:** ikke prøv å tegne bokstavelige kroppsfigurer igjen i denne appen, gå abstrakt. Samtidig kortet ned positur-varighetene fra 45-60 sek til 15-30 sek (brukeren: «umulig å måtte holde disse positurene så lenge») i `YOGA_ROUTINES_DEF`.
